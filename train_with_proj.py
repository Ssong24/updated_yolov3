...
def train(hyp):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            map_x = torch.zeros(imgs[0].shape, dtype=torch.uint8)
            for sk_i, sk_x in enumerate(map_x.numpy()):
                print(sk_x)
            input()
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            ...
            results, maps, class_names = test.test(cfg,
                                      data,
                                      batch_size=batch_size,
                                      imgsz=imgsz_test,
                                      model=ema.ema,
...
if __name__ == '__main__':
    ...
    if not opt.evolve:  # Train normally
        ...
        for _ in range(20):  # generations to evolve
            if os.path.exists(os.path.join(opt.output, 'evolve.txt')):  # if evolve.txt exists: select best hyps and mutate
                # Select parent(s)
                parent = 'single'  # parent selection method: 'single' or 'weighted'
                x = np.loadtxt(os.path.join(opt.output, 'evolve.txt'), ndmin=2)
                n = min(5, len(x))  # number of previous results to consider
                x = x[np.argsort(-fitness(x))][:n]  # top n mutations
                print('x: ', x)
                print('fitness(x): ', fitness(x))
                print('fitness(x).min(): ', fitness(x).min())
                ...
                if parent == 'single' or len(x) == 1:
                    ...
                    print('n: ', n)
                    print('w: ', w)
                    print('random.choices(range(n)): ', random.choices(range(n)))
            ...
            print_mutation(hyp, results, opt.bucket, opt.output)

            # Plot results
            plot_evolution_results(hyp, opt.output)
